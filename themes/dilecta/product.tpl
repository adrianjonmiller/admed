...
{include file="$tpl_dir./breadcrumb.tpl"}
	...
	{/if}
			...
			<div class="product-info{if Configuration::get('dilecta_custom_block_status') == '1' && Configuration::get('custom_block_text') != ''} custom-block{/if}" itemscope="" itemtype="http://schema.org/Product">
...
<meta itemprop="brand" content="{$product->manufacturer_name}">
		...
		<div id="image-block"{if $jqZoomEnabled} class="product-jqzoom"{else} class="product-no-jqzoom"{/if}>

		{if $have_image}

			<span id="view_full_size">

				{if $jqZoomEnabled}<a class="jqzoom" href="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_default')}" id="big_pic" title="$product->name|escape:'htmlall':'UTF-8'}">{/if}

				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}" title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" id="small_pic" itemprop="image"/>{if $jqZoomEnabled}</a>{/if}
		...
		{else}
			...
			<span id="view_full_size">
				...
				<img src="{$img_prod_dir}{$lang_iso}-default-large_default.jpg" id="bigpic" alt="" title="{$product->name|escape:'htmlall':'UTF-8'}"  itemprop="image"/>
				...
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}">
						...
						<a href="{$link->getImageLink($product->link_rewrite, $imageIds, thickbox_default)}" rel="other-views" class="thickbox {if $smarty.foreach.thumbnails.first}shown{/if}" title="{$image.legend|htmlspecialchars}">
							...
							<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default')}" alt="{$image.legend|htmlspecialchars}" itemprop="image"/>
				...
				<div class="right">
					...
					<div class="description">
						...
						<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
							...
							{l s='Reference:'} <span class="editable">{$product->reference|escape:'htmlall':'UTF-8'}</span>
...
{if $priceDisplay >= 0 && $priceDisplay <= 2}
    {if $currencyFormat == 1 || $currencyFormat == 3}
       <meta itemprop="priceCurrency" content="{$currency->iso_code}">  <span id="our_price_display" >{$currencySign}<span itemprop="price">{$productPrice|regex_replace:"/[\\{$currencySign}]/":""}</span></span>
    {else}
        <span id="our_price_display" itemprop="price">{$currencySign}{$productPrice|regex_replace:"/[\\{$currencySign}]/":""}</span> <meta itemprop="priceCurrency" content="{$currency->iso_code}"> {$currencySign}
    {/if}   
...
{/if}
						...
						</span> {if $product->specificPrice AND $product->specificPrice.reduction}{if $productPriceWithoutReduction > $productPrice}<span class="price-old" id="old_price_display">{convertPrice price=$productPriceWithoutRedution}</span>{/if}{/if}</p>
...
<div id="made_usa">
				...
				{/if}
						...
						{if $priceDisplay == 2}
							...
							<div><span id="pretaxe_price"><span id="pretaxe_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='tax excl.'}</span></div>
						...
						<div>{l s='instead of'} <span style="text-decoration: line-through;">{convertPrice price=$product->getNoPackPrice()}</span></div>
						...
						{if !empty($product->unity) && $product->unit_price_ratio > 0.000000}
							 ...
							 <div><span id="unit_price_display">{convertPrice price=$unit_price}</span> {l s='per'} {$product->unity|escape:'htmlall':'UTF-8'}</div>
		...
		{if Configuration::get('dilecta_product_social_share') != '0'}
...
<p class="clears" style="height:25px"></p>
				...
				{if Configuration::get('dilecta_custom_block_status') == '1' && Configuration::get('custom_block_text') != ''}
				...
				<div class="custom-block-product">
					...
					{Configuration::get('custom_block_text')}
...
{if isset($packItems) && $packItems|@count > 0}
	...
	<div class="block">
		...
		<h4 class="title_block">{l s='Pack content'}</h4>

		{include file="$tpl_dir./product-grid.tpl" products=$packItems}
