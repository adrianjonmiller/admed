...
include(dirname(__FILE__). '/nmi.php');
...
$params = array(
	...
	'x_test_request' => (bool)Configuration::get('NMI_DEMO'),
	...
	'x_login' => Tools::safeOutput(Configuration::get('NMI_LOGIN_ID')),

	'x_tran_key' => Tools::safeOutput(Configuration::get('NMI_KEY')),
...

{
	...
	$msg = 'NMI returned a malformed response for cart';
	...
	die('NMI returned a malformed response, aborted.');
...

{
	...
	case 1: // Payment accepted
		...
		$nmi->setTransactionDetail($response);

		$nmi->validateOrder((int)$cart->id,
	...
	case 4: // Hold for review
		...
		$nmi->validateOrder((int)$cart->id,
			...
			Configuration::get('NMI_HOLD_REVIEW_OS'), (float)$response[9],

			$nmi->displayName, $response[3], NULL, NULL, false, $customer->secure_key);
...
Tools::redirect($url.'id_module='.(int)$nmi->id.'&id_cart='.
