					...
					<!-- Product -->
						...
						<div>
							...
							<div class="left">
								...
								<div class="image">{if $product.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}{if isset($product.reduction) && $product.reduction}<div class="sale">{l s='Sale' mod='homefeatured'}</div>{/if}{/if}<a href="{$product.link}" title="{$product.name|escape:html:'UTF-8'}"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_default')}" alt="{$product.name|escape:html:'UTF-8'}" /></a></div>
								...
								<div class="on-hover">
									...
									{if ($product.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product.available_for_order AND !isset($restricted_country_mode) AND $product.minimal_quantity == 1 AND $product.customizable != 2 AND !$PS_CATALOG_MODE}	
										...
										{if ($product.quantity > 0 OR $product.allow_oosp)}						
											<a  href="{$product.link}" title="{l s='Add to cart' mod='homefeatured'}"  class="ajax_add_to_cart_button add-to-cart"></a>
										{else}
