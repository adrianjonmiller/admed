...
{include file="$tpl_dir./breadcrumb.tpl"}
	...
	{/if}
			...
			<div class="product-info{if Configuration::get('dilecta_custom_block_status') == '1' && Configuration::get('custom_block_text') != ''} custom-block{/if}">
		...
		{if $have_image}
			...
			<span id="view_full_size">
				...
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}" title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" id="small_pic" />{if $jqZoomEnabled}</a>{/if}
				...
				<div class="right">
					...
					<div class="description">
						...
						<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
							...
							{l s='Reference:'} <b>{$product->reference|escape:'htmlall':'UTF-8'}</b>
					...
					<div class="price">
						...
						<p><span id="our_price_display">{convertPrice price=$productPrice}</span> {if $product->specificPrice AND $product->specificPrice.reduction}{if $productPriceWithoutReduction > $productPrice}<span class="price-old" id="old_price_display">{convertPrice price=$productPriceWithoutRedution}</span>{/if}{/if}</p>
